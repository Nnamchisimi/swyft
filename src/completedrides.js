...
export default function CompletedRides({ rides }) {
  ...
  return (
    ...
    maxWidth: isDesktop ? "1000px" : "500px", // cap it at 1000px on desktop, 500px on mobile
      ...
      <List sx={{ p: 2 }}>
        {rides.length === 0 ? (
          <Typography variant="body2" color="text.secondary">No completed rides.</Typography>
        ) : (
          rides.map((ride) => (
                ...
                <ListItemText
                  ...
                  secondary={
                    <>
                      ...
                      <Typography variant="body2" color="textSecondary">Completed At: {new Date(ride.completed_at).toLocaleString()}</Typography>
